...
train_data = pd.read_csv('train_data_250.csv')
test_data = pd.read_csv('test_data_250.csv')
train_numerical_data = pd.read_csv('train_data_rbi_250.csv')
test_numerical_data = pd.read_csv('test_data_rbi_250.csv')

# KW
data_path='ro_paragraphs_250.csv'
...
train_data.rename(columns={'author': 'label', 'paragraph': 'text'}, inplace=True)
test_data.rename(columns={'author': 'label', 'paragraph': 'text'}, inplace=True)
...
class TextDataset(Dataset):
    def __init__(self, texts, labels, numerical_features, tokenizer):
        self.text_encodings = tokenizer(texts, truncation=True, padding=True, return_tensors='pt')
        self.labels = torch.tensor(labels, dtype=torch.long)
        self.numerical_features = torch.tensor(numerical_features.values, dtype=torch.float32)

    def __getitem__(self, idx):
        item = {
            'input_ids': self.text_encodings['input_ids'][idx].to(device),
            'attention_mask': self.text_encodings['attention_mask'][idx].to(device),
            'labels': self.labels[idx].to(device),
            'numerical_features': self.numerical_features[idx].to(device),
        }
        return item
...
for epoch in tqdm(range(training_args.num_train_epochs), desc="Training"):
    for batch in torch.utils.data.DataLoader(train_dataset, batch_size=32, shuffle=True):
        ...
        labels = batch['labels']
        numerical_features[torch.isnan(numerical_features)] = 0  
        numerical_features[torch.isinf(numerical_features)] = 0  

        # num features before normalization
        #print("before normalization:", numerical_features)

        numerical_features = (numerical_features - numerical_features.mean()) / numerical_features.std()

        # num features after normalization
        #print("after normalization:", numerical_features)
